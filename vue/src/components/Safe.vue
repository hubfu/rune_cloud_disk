          ...
          <template slot-scope="scope">
            ...
            <el-menu style="border-right: 0;">
              ...
              <el-menu-item index="4">
                 <el-button  class="moreBtn" @click="removeInSafe(scope.row.id)">移出保险箱</el-button>
              </el-menu-item>
...
export default {
  ...
  created:function(){
   this.$axios
      .get('/file/safe/list/'+ localStorage.getItem('userid')+'/1')
      .then((res) => {
        var result = res.data
  ...
  methods: {
    goBack() {
     ...
     currentChange(val) {
      this.$axios
        .get(
          '/file/safe/list/' + localStorage.getItem('userid') + '/' + val
        )
        .then((res) => {
          var result = res.data
          this.pageInfo = result.model
    ...
    renameOpen(filename){
     ...
     removeInSafe(fileId){
      console.log(fileId)
       this.$axios.get("/file/safe/remove"+'/'+fileId)
      .then(res=>{
        var result=res.data;
        if(result.code==200){
          this.$message.success(result.message)
